local TerminalSoundPlayer = TerminalSoundPlayer
local warExtended = warExtended
local WINDOW_NAME = "TerminalSoundPlayer"
local SOUND_COMBO_BOX             = 1
local SOUND_PLAY_BUTTON            = 2
local TITLEBAR               = 3
...
function WINDOW:Create()
    ...
    if frame then
        frame.m_Windows = {
            ...
            [SOUND_COMBO_BOX] = ComboBox:CreateFrameForExistingWindow(WINDOW_NAME.."SoundComboBox"),
            [SOUND_PLAY_BUTTON] = ButtonFrame:CreateFrameForExistingWindow(WINDOW_NAME .. "PlayButton"),
        }

        local win      = frame.m_Windows

        win[SOUND_PLAY_BUTTON].OnLButtonUp = function(self, ...)
            local _, soundName = win[SOUND_COMBO_BOX]:GetSelectedMenuItem()
            warExtended:PlaySound(soundName)
        end

        win[TITLEBAR]:SetText(L"Sound Player")
        win[SOUND_PLAY_BUTTON]:SetText(L"Play")
        win[SOUND_COMBO_BOX]:AddTable(TerminalSoundPlayer:GetSettings().sounds)

        --warExtended:GetConstants("Sounds")
    end
end
