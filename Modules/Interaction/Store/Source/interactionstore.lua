...
local function getSearchText()
  local searchText = TextEditBoxGetText("EA_Window_InteractionStoreSearchBox")
  return searchText
end
...
function EA_Window_InteractionStore.CreateFilteredList()
  ...
  do
	...
	if isSearchText then
	  ...
	  if not warExtended:MatchSearch(itemData, searchText) then
		show = false
	  end
...
function EA_Window_InteractionStore.OnItemRButtonUp( flags, x, y )
  ...
  elseif warExtended:IsFlagName("isAltPressed", flags) then
	local maxStackCount = EA_Window_InteractionStore.displayData[dataIdx].capacity
	EA_Window_InteractionStore.ConfirmThenBuyItem( dataIdx, maxStackCount )
  else
